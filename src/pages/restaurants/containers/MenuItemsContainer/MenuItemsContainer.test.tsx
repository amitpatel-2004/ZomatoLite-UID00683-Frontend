...
describe('MenuItemsContainer', () => {
  ...
  it('should render the section title and empty state', async () => {
    renderWithStore(<MenuItemsContainer isOwner={false} restaurantId="1" />);
  ...
  it('should render the Add Item button when isOwner is true', () => {
    renderWithStore(<MenuItemsContainer isOwner={true} restaurantId="1" />);
  ...
  it('should upload the image and create the menu item when an image is provided', async () => {
    mockGetUploadUrl.mockResolvedValue({
      uploadUrl: 'https://GCS.com/upload',
      imagePath: 'menu-items/pizza.png',
    });
    ...
    await user.upload(fileInput, new File(['pic'], 'pizza.png', { type: 'image/png' }));
    ...
    expect(mockCreate).toHaveBeenCalledWith(
      '1',
      expect.objectContaining({ imagePath: 'menu-items/pizza.png' }),
    );
  });

  it('should show an error and not create the menu item when the image upload fails', async () => {
    mockGetUploadUrl.mockRejectedValue(new Error('Unknown error occured.'));
    const user = userEvent.setup();
    ...
    expect(await screen.findByText('Unknown error occured.')).toBeVisible();
